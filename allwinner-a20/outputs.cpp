...
void VideoOutput::aa_touch_event(SDL_Window *window, HU::TouchInfo::TOUCH_ACTION action, float x, float y) {
    int windowW, windowH;
...
gboolean VideoOutput::sdl_poll_event() {
	...
	case SDL_FINGERMOTION:
            ...
            aa_touch_event(SDL_GetWindowFromID(windowId), HU::TouchInfo::TOUCH_ACTION_DRAG, tfevent->x, tfevent->y);
            break;
        case SDL_MOUSEMOTION:
            mmevent = &event.motion;
            if (mmevent->state & SDL_BUTTON_LMASK) {
                aa_touch_event(SDL_GetWindowFromID(mmevent->windowID), HU::TouchInfo::TOUCH_ACTION_DRAG, (float) mmevent->x, (float) mmevent->y);
            }
            break;
	case SDL_FINGERDOWN:
            tfevent = &event.tfinger;
            aa_touch_event(SDL_GetWindowFromID(windowId), HU::TouchInfo::TOUCH_ACTION_PRESS, tfevent->x, tfevent->y);
            break;
        case SDL_MOUSEBUTTONDOWN:
            mbevent = &event.button;
            if (mbevent->button == SDL_BUTTON_LEFT) {
                aa_touch_event(SDL_GetWindowFromID(mbevent->windowID), HU::TouchInfo::TOUCH_ACTION_PRESS, (float) mbevent->x, (float) mbevent->y);
            }
            break;
	case SDL_FINGERUP:
            tfevent = &event.tfinger;
            aa_touch_event(SDL_GetWindowFromID(windowId), HU::TouchInfo::TOUCH_ACTION_RELEASE, tfevent->x, tfevent->y);
            break;
        case SDL_MOUSEBUTTONUP:
            mbevent = &event.button;
            if (mbevent->button == SDL_BUTTON_LEFT) {
                aa_touch_event(SDL_GetWindowFromID(mbevent->windowID), HU::TouchInfo::TOUCH_ACTION_RELEASE, (float) mbevent->x, (float) mbevent->y);
            }
            break;
        case SDL_KEYDOWN:
        case SDL_KEYUP:
...
VideoOutput::VideoOutput(DesktopEventCallbacks* callbacks) : callbacks(callbacks) {
    ...
    window = SDL_CreateWindow("Android Auto",
                              ...
                              //(int) (1024 * g_dpi_scalefactor), (int) (600 * g_dpi_scalefactor),
                          #else
                              (int) (1024 * g_dpi_scalefactor), (int) (600 * g_dpi_scalefactor),
                          #endif
